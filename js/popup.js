(function(window) {
              ...
              ScatterJS.scatter
                ...
                .then(async info => {
                  let email = i('xprime').value.length > 12
                    ...
                    : i('xprime').value;
                  const data = 'EOS Layers Login using ' + email;
                  try {
  ...
  async function _getFirstBountyPledge() {
    ...
    for(let response of responses) {
      try {
        const txHash = response.transactionFullHash,
        noteRequest = `${GLOBAL['node']}/nxt?requestType=getTaggedData&chain=ignis&transactionFullHash=${txHash}`,
    ...
    try {
      ...
      if(!nodeRequest.errorDescription) { 
        for(let response of nodeResponse.data) { 
          ...
          const hash = response.transactionFullHash;
          const {data : bountyAnswer} = await getRequest(`${GLOBAL['node']}/nxt?requestType=getTaggedData&chain=ignis&transactionFullHash=${hash}`); 
